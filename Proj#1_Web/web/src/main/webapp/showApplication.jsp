...
<section class="container">
    ...
    <div class ="card bg-light mt-3">
        <div class="card-body bg-light">
            ...
            <div class="row">
                <div class="col-sm-1">
                    <%= classesDAO.getClassno(applications.getClass_id())%>
                </div>
                <div class="col-sm">
                    <%= classesDAO.getCourseid(applications.getClass_id())%>
                </div>
                <div class="col-sm-2">
                    <%= classesDAO.getName(applications.getClass_id())%>
                </div>
                <div class="col-sm-1">
                    <%= classesDAO.getCredit(applications.getClass_id())%>
                </div>
                <div class="col-sm">
                    <%= lecturer.getLecturerName(classesDAO.getLecturerid(applications.getClass_id()))%>
                </div>
                <div class="col-sm-2">
                    <%= time.getTimeBegin(time.getTimeId(applications.getClass_id()))%>
                </div>
                <div class="col-sm-2">
                    <%= applicationDAO.getHowManyApplication(applications.getClass_id())%>/<%= classesDAO.getPersonMax(applications.getClass_id())%>
                </div>
                <div class="col-sm-2">
                    <%= building.getName(room.getBuildingId(classesDAO.getRoomid(applications.getClass_id())))%> <%= classesDAO.getRoomid(applications.getClass_id())%>호
                </div>
                <div class="col-sm-1">
                </div>
            </div>
...
<%--    수강취소 버튼--%>
    <a onclick="return confird('취소하시겠습니까?')" href="./applicationCancleAction.jsp?class_id=<%= applications.getClass_id() %>">수강취소</a>
